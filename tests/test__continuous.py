...
class TestNormal:
    ...
    def test_kl_divergence_starndard_normal(self, init_tensor) -> None:
    ...
    def test_parameters(self, init_tensor) -> None:
        """Test `parameters`."""
        loc, scale = torch.chunk(init_tensor, 2, dim=-1)
        dist = Normal(loc, scale)
...
class TestGMM:
    ...
    def test_mean(self, init_tensor) -> None:
        """Test `mean`."""
    ...
    def test_sample(self, init_tensor) -> None:
        """Test `sample()`."""
        probs, loc, scale = torch.chunk(init_tensor, 3, dim=-1)
        probs = torch.softmax(probs, dim=-1)
        dist = GMM(probs, loc, scale)
    ...
    def test_rsample(self, init_tensor) -> None:
        """Test `rsample()`."""
        probs, loc, scale = torch.chunk(init_tensor, 3, dim=-1)
        probs = torch.softmax(probs, dim=-1)
        dist = GMM(probs, loc, scale)
    ...
    def test_log_prob(self, init_tensor) -> None:
        """Test `log_prob()`."""
        probs, loc, scale = torch.chunk(init_tensor, 3, dim=-1)
        probs = torch.softmax(probs, dim=-1)
        dist = GMM(probs, loc, scale)
